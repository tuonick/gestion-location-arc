...
function locarc_fe_register_assets() {
    ...
    wp_register_style('locarc-fe-dashboard', plugins_url('../assets/frontend-dashboard.css', __FILE__), [], file_exists($css) ? filemtime($css) : LOCARC_VERSION);
    wp_register_script('locarc-fe-dashboard', plugins_url('../assets/frontend-dashboard.js', __FILE__), [], file_exists($js) ? filemtime($js) : LOCARC_VERSION, true);
}
...
function locarc_fe_shortcode() {
    ...
    $ct_config = locarc_contract_types_config();
    $ct_js = [];
    foreach ($ct_config as $key => $row) {
        $ct_js[] = ['key' => $key, 'label' => $row['label'], 'price' => floatval($row['price'] ?? 0)];
    }

    wp_localize_script('locarc-fe-dashboard', 'LOCARC_FE', [
...
function locarc_fe_section_contracts() {
    ...
    if (!empty($missing)) locarc_prime_member_names($missing);
    ...
    foreach ($rows as $r) {
        if (!intval($r['is_paid'] ?? 0) && ($r['contract_type'] ?? '') !== 'pret') $unpaid++;
        $end_ts = strtotime($r['end_date'] ?? '');
        if ($end_ts && ($end_ts - $today) >= 0 && ($end_ts - $today) <= $three_weeks) $expiring++;
    }
    ?>
        ...
        <tbody>
          <?php foreach ($rows as $r) : ?>
            ...
            if (trim((string)($r['first_name'] ?? '')) === '' && trim((string)($r['last_name'] ?? '')) === '') {
                ...
                $r['first_name'] = $fn; $r['last_name'] = $ln;
            }
            ...
            $row_cls = '';
            if ($is_expiring) $row_cls = 'is-expiring';
            elseif ($is_expired) $row_cls = 'is-expired';
            ?>
            <tr class="locarc-fe__row <?php echo esc_attr($row_cls); ?>"
              ...
              <td><?php echo esc_html($full_name ?: $r['licence']); ?></td>
              <td><?php echo esc_html(locarc_contract_type_label($r['contract_type'])); ?></td>
              <td class="<?php echo $is_expiring ? 'locarc-fe__cell--warn' : ($is_expired ? 'locarc-fe__cell--danger' : ''); ?>">
              ...
              <td>
                ...
                <?php else : ?>
                  <button class="locarc-fe__btn locarc-fe__btn--sm locarc-fe__btn--outline js-genpdf" data-id="<?php echo intval($r['id']); ?>">Générer</button>
                <?php endif; ?>
              </td>
              <td class="locarc-fe__actions">
                <button class="locarc-fe__btn locarc-fe__btn--sm locarc-fe__btn--outline js-edit-contract" data-id="<?php echo intval($r['id']); ?>">Modifier</button>
                <button class="locarc-fe__btn locarc-fe__btn--sm locarc-fe__btn--outline js-renew-contract" data-id="<?php echo intval($r['id']); ?>">Renouveler</button>
                <button class="locarc-fe__btn locarc-fe__btn--sm locarc-fe__btn--danger js-archive" data-id="<?php echo intval($r['id']); ?>">Archiver</button>
              </td>
            </tr>
          <?php endforeach; ?>
          ...
          <?php foreach ($archived as $r) : ?>
            ...
            <tr class="locarc-fe__row locarc-fe__row--archived"
              ...
              <td class="locarc-fe__actions">
                <button class="locarc-fe__btn locarc-fe__btn--sm locarc-fe__btn--outline js-restore" data-id="<?php echo intval($r['id']); ?>">Restaurer</button>
              </td>
            </tr>
          <?php endforeach; ?>
        </tbody>
...
function locarc_fe_section_rented() {
    ...
    $with_both = 0;
    foreach ($rows as $r) {
        if (!empty($r['handle_identifier']) && !empty($r['branches_identifier'])) $with_both++;
    }
    ?>
    <div class="locarc-fe__section-header">
    ...
    <div class="locarc-fe__toolbar">
      ...
      <?php
      $rented_types = [];
      foreach ($rows as $r) { $rented_types[$r['contract_type'] ?? ''] = locarc_contract_type_label($r['contract_type'] ?? ''); }
...
function locarc_fe_section_branches() {
    ...
    $total = count($rows); $dispo = 0; $rented = 0; $unavail = 0;
    foreach ($rows as $r) {
        ...
        <tbody>
          <?php foreach ($rows as $r) : ?>
            ...
            if (!empty($r['renter_licence'])) {
                ...
                if ($renter === '') $renter = $r['renter_licence'];
            }
            $avail_val  = intval($r['is_available'] ?? 0);
            $avail_map  = [1 => 'ok', 0 => 'warn', 2 => 'info', 3 => 'danger', 4 => 'danger', 5 => 'danger'];
...
function locarc_fe_section_handles() {
    ...
    $total = count($rows); $dispo_r = 0; $dispo_l = 0; $rented = 0;
    foreach ($rows as $r) {
        $s = intval($r['is_available'] ?? 0);
        if ($s === 1) {
            if (trim($r['handedness'] ?? '') === 'Gauche') $dispo_l++; else $dispo_r++;
        } elseif ($s === 0) $rented++;
    }
    ?>
    <div class="locarc-fe__section-header">
...
function locarc_fe_icon($section) {
    ...
    echo $icons[$section] ?? ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
