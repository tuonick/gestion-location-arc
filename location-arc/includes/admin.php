...
function locarc_admin_assets($hook) {
    ...
    $contract_config = locarc_contract_types_config();
    $contract_prices = [];
    $contract_labels = [];
...
function locarc_tabs() {
    ...
    echo '<span class="locarc-tab-group-pill' . (!$in_gestion ? ' is-active' : '') . '" data-group="admin">Administration</span>';
    echo '</div>';
    ...
    echo '<h2 class="nav-tab-wrapper locarc-tabs locarc-tabs--group-admin' . (!$in_gestion ? '' : ' locarc-tabs--hidden') . '">';
...
function locarc_render_settings() {
    ...
    if (isset($_POST['locarc_save_contract_settings'])) {
        ...
        $posted_labels = isset($_POST['contract_labels']) && is_array($_POST['contract_labels']) ? $_POST['contract_labels'] : [];
        $posted_prices = isset($_POST['contract_prices']) && is_array($_POST['contract_prices']) ? $_POST['contract_prices'] : [];
        $new_config = [];
        ...
        // Dompdf autoload path (optional — leave empty to use auto-detection).
        $dompdf_path = sanitize_text_field(wp_unslash($_POST['locarc_dompdf_autoload_path'] ?? ''));
        update_option('locarc_dompdf_autoload_path', $dompdf_path);
    ...
    $saved_dompdf_path   = (string) get_option('locarc_dompdf_autoload_path', '');
    ...
    foreach ($defaults as $key => $def) {
        ...
        echo '<td><input type="number" step="0.01" min="0" name="contract_prices[' . esc_attr($key) . ']" value="' . esc_attr($row['price']) . '" ' . $disabled . ' />' . $hint . '</td>';
        echo '</tr>';
    }
    echo '</tbody></table>';

    echo '<h2 style="margin-top:32px;">Génération PDF (Dompdf)</h2>';
    echo '<p>Le plugin recherche Dompdf dans l\'ordre suivant :</p>';
    echo '<ol style="list-style:decimal;margin-left:20px;">';
    echo '<li>Chemin configuré ci-dessous (si renseigné)</li>';
    echo '<li><code>' . esc_html(trailingslashit(ABSPATH) . 'vendor/autoload.php') . '</code> — Composer racine WordPress</li>';
    echo '<li><code>' . esc_html(trailingslashit(WP_CONTENT_DIR) . 'vendor/autoload.php') . '</code> — Composer racine wp-content</li>';
    echo '<li><code>' . esc_html(trailingslashit(WP_CONTENT_DIR) . 'vendor/dompdf/autoload.inc.php') . '</code> — dossier dompdf déplacé dans wp-content/vendor</li>';
    echo '<li><code>' . esc_html(trailingslashit(LOCARC_PLUGIN_DIR) . 'vendor/autoload.php') . '</code> — Composer dans le dossier du plugin</li>';
    echo '<li><code>' . esc_html(trailingslashit(LOCARC_PLUGIN_DIR) . 'vendor/dompdf/autoload.inc.php') . '</code> — dossier dompdf dans le plugin (ancien emplacement)</li>';
    echo '</ol>';
    echo '<p class="description">Pour installer Dompdf via Composer à la racine WordPress : <code>composer require dompdf/dompdf</code></p>';
    echo '<table class="form-table"><tbody>';
    echo '<tr><th scope="row"><label for="locarc-dompdf-path">Chemin personnalisé vers <code>autoload.php</code></label></th><td>';
    echo '<input id="locarc-dompdf-path" type="text" class="large-text" name="locarc_dompdf_autoload_path" value="' . esc_attr($saved_dompdf_path) . '" placeholder="/chemin/absolu/vers/vendor/autoload.php" />';
    echo '<p class="description">Laisser vide pour utiliser la détection automatique. Chemin absolu vers le fichier <code>autoload.php</code> de Dompdf ou de votre installation Composer partagée.</p>';
    echo '</td></tr>';
    echo '</tbody></table>';
...
function locarc_render_alerts_settings() {
    ...
    if (isset($_POST['locarc_save_alerts'])) {
        ...
        $admin_to = sanitize_email($_POST['alerts_admin_to'] ?? '');
...
function locarc_member_display_name($licence) {
    global $wpdb;
    $t = locarc_tables();
    $m = $wpdb->get_row($wpdb->prepare("SELECT first_name, last_name FROM {$t['members']} WHERE licence=%s", $licence), ARRAY_A);
...
function locarc_prime_member_names($licences) {
    ...
    $rows = $wpdb->get_results($wpdb->prepare($sql, ...$need), ARRAY_A);
    $found = [];
    foreach ($rows as $r) {
    ...
    $u_rows = $wpdb->get_results($wpdb->prepare($u_sql, ...$remaining), ARRAY_A);
    ...
    if (!empty($user_ids)) {
        ...
        $m_rows = $wpdb->get_results($wpdb->prepare($m_sql, ...$user_ids), ARRAY_A);
...
function locarc_is_equipment_assigned($identifier, $kind, $exclude_contract_id = 0) {
    global $wpdb;
    $t = locarc_tables();
    $col = ($kind === 'branches') ? 'branches_identifier' : 'handle_identifier';
...
function locarc_render_contracts() {
    ...
    $active_count = $wpdb->get_var("SELECT COUNT(*) FROM {$t['contracts']} WHERE status='active'");
    // Revenu potentiel = somme des contrats actifs, indépendamment du statut payé.
    // Personnalisé : utilise custom_price (0 si vide).
    $active_contracts = $wpdb->get_results("SELECT contract_type, custom_price FROM {$t['contracts']} WHERE status='active'", ARRAY_A);
    $potential = 0;
    ...
    foreach (['complet','arc_nu','branches','jeune','personnalise','pret'] as $k) {
        $label = locarc_contract_type_label($k);
        $val = intval($counts[$k] ?? 0);
        echo '<div class="locarc-kpi-chip">'
            . '<span class="locarc-kpi-chip-val">' . $val . '</span>'
    ...
    foreach ($rows as $r) {
        ...
        echo '<tr class="' . esc_attr($row_class) . '" data-id="' . esc_attr($r['id']) . '"'
            . (!empty($_GET['new_id']) && intval($_GET['new_id'])===intval($r['id']) ? ' data-new="1"' : '') . '>'
            . '<td>' . intval($i++) . '</td>'
...
function locarc_render_rented() {
    global $wpdb;
    $t = locarc_tables();
    $rows = $wpdb->get_results("SELECT c.*, m.first_name, m.last_name FROM {$t['contracts']} c LEFT JOIN {$t['members']} m ON m.licence=c.licence WHERE c.status='active' ORDER BY m.last_name ASC, m.first_name ASC", ARRAY_A);
...
function locarc_render_imports() {
    ...
    if (isset($_GET['imported'])) {
        echo '<div class="notice notice-success is-dismissible"><p>✅ Import terminé : <strong>' . intval($_GET['imported']) . '</strong> lignes traitées.</p></div>';
    }
    if (!empty($_GET['import_error'])) {
        echo '<div class="notice notice-error is-dismissible"><p>❌ Import impossible : ' . esc_html(wp_unslash($_GET['import_error'])) . '</p></div>';
    }
...
ACSIM"));
    ...
    echo '<p class="description">Adresse(s) en copie pour l’envoi du contrat.</p></td></tr>';
...
add_action('admin_post_locarc_import_members', function() {
    ...
    if (is_wp_error($validate)) {
        wp_redirect(admin_url('admin.php?page=locarc&tab=imports&import_error=' . rawurlencode($validate->get_error_message())));
        exit;
    }
    $count = locarc_import_members_from_csv($_FILES['csv']['tmp_name']);
    if (is_wp_error($count)) {
        wp_redirect(admin_url('admin.php?page=locarc&tab=imports&import_error=' . rawurlencode($count->get_error_message())));
        exit;
    }
    wp_redirect(admin_url('admin.php?page=locarc&tab=imports&imported=' . intval($count)));
    exit;
});
add_action('admin_post_locarc_import_branches', function() {
    ...
    $count = locarc_import_matos_from_csv($_FILES['csv']['tmp_name'], 'branches');
    if (is_wp_error($count)) {
        wp_redirect(admin_url('admin.php?page=locarc&tab=imports&import_error=' . rawurlencode($count->get_error_message())));
        exit;
    }
...
add_action('admin_post_locarc_import_handles', function() {
    ...
    $count = locarc_import_matos_from_csv($_FILES['csv']['tmp_name'], 'handles');
    if (is_wp_error($count)) {
        wp_redirect(admin_url('admin.php?page=locarc&tab=imports&import_error=' . rawurlencode($count->get_error_message())));
        exit;
    }
...
function locarc_ajax_get_item() {
    ...
    $id = intval($_GET['id'] ?? 0);
    if (!$id || !in_array($kind, ['branches','handles','contracts','members'], true)) wp_send_json_error('bad_request', 400);
...
function locarc_ajax_save_item() {
    ...
    if (!in_array($kind, ['branches','handles','members'], true)) wp_send_json_error('bad_kind', 400);
    ...
    $previous = null;
    if ($id > 0) {
        $previous = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$table} WHERE id=%d", $id), ARRAY_A);
    ...
    $saved = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$table} WHERE id=%d", $saved_id), ARRAY_A);
...
function locarc_ajax_delete_item() {
    ...
    $row = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t[$kind]} WHERE id=%d", $id), ARRAY_A);
    if (!$row) wp_send_json_error('not_found', 404);
    $object_type = ($kind === 'branches') ? 'branch' : 'handle';
...
function locarc_ajax_update_paid() {
    ...
    $before = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t['contracts']} WHERE id=%d", $id), ARRAY_A);
    if (!$before) wp_send_json_error('not_found', 404);
    $wpdb->update($t['contracts'], ['is_paid'=>$paid, 'updated_at'=>current_time('mysql')], ['id'=>$id], ['%d','%s'], ['%d']);
    $after = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t['contracts']} WHERE id=%d", $id), ARRAY_A);
    if ($after) {
...
function locarc_ajax_archive_contract() {
    ...
    $before = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t['contracts']} WHERE id=%d", $id), ARRAY_A);
    if (!$before) wp_send_json_error('not_found', 404);
    $wpdb->update($t['contracts'], ['status'=>'archived', 'updated_at'=>current_time('mysql')], ['id'=>$id], ['%s','%s'], ['%d']);
    locarc_sync_availability();
    $after = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t['contracts']} WHERE id=%d", $id), ARRAY_A);
    if ($after) {
...
function locarc_ajax_restore_contract() {
    ...
    $before = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t['contracts']} WHERE id=%d", $id), ARRAY_A);
    if (!$before) wp_send_json_error('not_found', 404);
    $wpdb->update($t['contracts'], ['status'=>'active', 'updated_at'=>current_time('mysql')], ['id'=>$id], ['%s','%s'], ['%d']);
    locarc_sync_availability();
    $after = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t['contracts']} WHERE id=%d", $id), ARRAY_A);
    if ($after) {
...
function locarc_ajax_delete_contract_permanent() {
    ...
    $row = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t['contracts']} WHERE id=%d", $id), ARRAY_A);
    $status = $row['status'] ?? '';
    ...
    $wpdb->delete($t['contracts'], ['id'=>$id], ['%d']);
    wp_send_json_success(true);
}
...
function locarc_ajax_autocomplete() {
    ...
    if (!in_array($kind, ['branches','handles','members','members_by_firstname'], true)) wp_send_json_error('bad_kind', 400);
    ...
    if ($kind === 'members') {
        ...
        $u_sql = "
            ...
            LIMIT 20
        ";
        $u_rows = $wpdb->get_results($wpdb->prepare($u_sql, ...$u_params), ARRAY_A);
        foreach ($u_rows as $u) {
...
function locarc_ajax_get_by_identifier() {
    ...
    if (!$identifier || !in_array($kind, ['branches','handles'], true)) wp_send_json_error('bad_request', 400);
    $row = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t[$kind]} WHERE identifier=%s", $identifier), ARRAY_A);
    if (!$row) {
        // Fallback: accept identifier without dashes
        $compact = preg_replace('/-+/', '', $identifier);
        if ($compact !== $identifier) {
            $row = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t[$kind]} WHERE REPLACE(identifier,'-','')=%s", $compact), ARRAY_A);
        }
    }
    if (!$row) wp_send_json_error('not_found', 404);
...
function locarc_ajax_get_member_by_licence() {
    ...
    $m = $wpdb->get_row($wpdb->prepare("SELECT licence, first_name, last_name, email, phone, address1, postal_code, city FROM {$t['members']} WHERE licence=%s", $licence), ARRAY_A);
    if ($m) {
...
function locarc_ajax_save_contract() {
    ...
    $type = sanitize_key($_POST['contract_type'] ?? 'complet');
    $start = sanitize_text_field($_POST['start_date'] ?? wp_date('Y-m-d'));
    $end = sanitize_text_field($_POST['end_date'] ?? wp_date('Y-m-d', strtotime('+1 year', strtotime($start))));
    $handle_id = sanitize_text_field($_POST['handle_identifier'] ?? '');
    $branches_id = sanitize_text_field($_POST['branches_identifier'] ?? '');
    // Equipment characteristics (optional; used for 'Prêt' or cached display/PDF)
    $handle_brand = sanitize_text_field($_POST['handle_brand'] ?? '');
    $handle_model = sanitize_text_field($_POST['handle_model'] ?? '');
    $handle_size = ($_POST['handle_size'] ?? '') === '' ? null : intval($_POST['handle_size']);
    $handle_handedness = sanitize_text_field($_POST['handle_handedness'] ?? '');

    $branches_brand = sanitize_text_field($_POST['branches_brand'] ?? '');
    $branches_model = sanitize_text_field($_POST['branches_model'] ?? '');
    $branches_size = ($_POST['branches_size'] ?? '') === '' ? null : intval($_POST['branches_size']);
    $branches_power = ($_POST['branches_power'] ?? '') === '' ? null : intval($_POST['branches_power']);
    ...
    $caution_amount_raw = $_POST['caution_amount'] ?? '';
    $caution_amount = ($caution_amount_raw === '' ? null : floatval(str_replace(',', '.', sanitize_text_field($caution_amount_raw))));
    $payment_due_1 = sanitize_text_field($_POST['payment_due_1'] ?? '');
    $payment_due_2 = sanitize_text_field($_POST['payment_due_2'] ?? '');
    $payment_due_3 = sanitize_text_field($_POST['payment_due_3'] ?? '');
    $payment_due_4 = sanitize_text_field($_POST['payment_due_4'] ?? '');
    ...
    $paid = intval($_POST['is_paid'] ?? 0) ? 1 : 0;
    $custom_price_raw = $_POST['custom_price'] ?? '';
    ...
    if ($id > 0) {
        $current_contract = $wpdb->get_row($wpdb->prepare(
            "SELECT id, handle_identifier, branches_identifier FROM {$t['contracts']} WHERE id=%d",
            $id
        ), ARRAY_A);
    }
    ...
    if ($handle_id) {
        $h = $wpdb->get_row($wpdb->prepare("SELECT brand, model, size, handedness FROM {$t['handles']} WHERE identifier=%s", $handle_id), ARRAY_A);
        if ($h) {
    ...
    if ($branches_id) {
        $b = $wpdb->get_row($wpdb->prepare("SELECT brand, model, size, power FROM {$t['branches']} WHERE identifier=%s", $branches_id), ARRAY_A);
        if ($b) {
...
// Build contract number only on create
    if ($id === 0) {
        ...
        $saved_contract = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t['contracts']} WHERE id=%d", $new_id), ARRAY_A);
        if ($saved_contract) {
            $summary = [];
            foreach (locarc_log_contract_field_labels() as $field => $label) {
        ...
        $before_contract = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t['contracts']} WHERE id=%d", $id), ARRAY_A);
        $res = $wpdb->update($t['contracts'], $data, ['id'=>$id]);
        if ($wpdb->last_error) {
        ...
        $after_contract = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t['contracts']} WHERE id=%d", $id), ARRAY_A);
...
function locarc_ajax_send_contract_email() {
    ...
    $id = intval($_POST['id'] ?? 0);
    if (!$id) wp_send_json_error('bad_request', 400);
    $c = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t['contracts']} WHERE id=%d", $id), ARRAY_A);
...
function locarc_ajax_update_contract_pricing() {
    ...
    $t = locarc_tables();
    $id = intval($_POST['id'] ?? 0);
    $type = sanitize_key($_POST['contract_type'] ?? '');
    $custom_price_raw = $_POST['custom_price'] ?? '';
    $custom_price = ($custom_price_raw === '' ? null : floatval(str_replace(',', '.', sanitize_text_field($custom_price_raw))));
...
function locarc_ajax_renew_contract() {
    ...
    $old = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t['contracts']} WHERE id=%d", $id), ARRAY_A);
    ...
    $licence = sanitize_text_field($_POST['licence'] ?? ($old['licence'] ?? ''));
    $type = sanitize_key($_POST['contract_type'] ?? ($old['contract_type'] ?? 'complet'));
    $start = sanitize_text_field($_POST['start_date'] ?? ($old['end_date'] ?? wp_date('Y-m-d')));
    $end = sanitize_text_field($_POST['end_date'] ?? wp_date('Y-m-d', strtotime('+1 year', strtotime($start))));
    $handle_id = sanitize_text_field($_POST['handle_identifier'] ?? ($old['handle_identifier'] ?? ''));
    $branches_id = sanitize_text_field($_POST['branches_identifier'] ?? ($old['branches_identifier'] ?? ''));

    $handle_brand = sanitize_text_field($_POST['handle_brand'] ?? ($old['handle_brand'] ?? ''));
    $handle_model = sanitize_text_field($_POST['handle_model'] ?? ($old['handle_model'] ?? ''));
    $handle_size = ($_POST['handle_size'] ?? '') === '' ? (($old['handle_size'] ?? '') === '' ? null : intval($old['handle_size'])) : intval($_POST['handle_size']);
    $handle_handedness = sanitize_text_field($_POST['handle_handedness'] ?? ($old['handle_handedness'] ?? ''));

    $branches_brand = sanitize_text_field($_POST['branches_brand'] ?? ($old['branches_brand'] ?? ''));
    $branches_model = sanitize_text_field($_POST['branches_model'] ?? ($old['branches_model'] ?? ''));
    $branches_size = ($_POST['branches_size'] ?? '') === '' ? (($old['branches_size'] ?? '') === '' ? null : intval($old['branches_size'])) : intval($_POST['branches_size']);
    $branches_power = ($_POST['branches_power'] ?? '') === '' ? (($old['branches_power'] ?? '') === '' ? null : intval($old['branches_power'])) : intval($_POST['branches_power']);

    $payment_method = sanitize_key($_POST['payment_method'] ?? ($old['payment_method'] ?? ''));
    if (!in_array($payment_method, ['', 'cheque', 'carte_bancaire', 'helloasso', 'especes'], true)) $payment_method = '';
    $caution_amount_raw = $_POST['caution_amount'] ?? ($old['caution_amount'] ?? '');
    ...
    $payment_due_1 = sanitize_text_field($_POST['payment_due_1'] ?? ($old['payment_due_1'] ?? ''));
    $payment_due_2 = sanitize_text_field($_POST['payment_due_2'] ?? ($old['payment_due_2'] ?? ''));
    $payment_due_3 = sanitize_text_field($_POST['payment_due_3'] ?? ($old['payment_due_3'] ?? ''));
    $payment_due_4 = sanitize_text_field($_POST['payment_due_4'] ?? ($old['payment_due_4'] ?? ''));
    if ($payment_method !== 'cheque') {
        $payment_due_1 = $payment_due_2 = $payment_due_3 = $payment_due_4 = '';
    }
    ...
    $paid = intval($_POST['is_paid'] ?? 0) ? 1 : 0;
    $custom_price_raw = $_POST['custom_price'] ?? ($old['custom_price'] ?? '');
    ...
    $new_contract = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t['contracts']} WHERE id=%d", $new_id), ARRAY_A);
    $archived_contract = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$t['contracts']} WHERE id=%d", $id), ARRAY_A);
...
function locarc_render_licencies(){
    ...
    $t = locarc_tables();
    $table = $t['members'];
    $rows = $wpdb->get_results("SELECT * FROM {$table} ORDER BY last_name ASC, first_name ASC", ARRAY_A);
