...
// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- All DB queries use locarc_tables() = $wpdb->prefix-based table names, never user input. Direct queries required for custom plugin tables.
...
function locarc_render_settings() {
    ...
    echo '<h2 style="margin-top:32px;">Génération PDF</h2>';
    echo '<p>La génération PDF utilise <strong>LocarcPDF</strong>, un générateur intégré au plugin (aucune dépendance externe).</p>';
    $fpdf_path = trailingslashit(LOCARC_PLUGIN_DIR) . 'vendor/fpdf/LocarcPDF.php';
    if (file_exists($fpdf_path)) {
        echo '<p>✅ <strong>LocarcPDF détecté</strong> et opérationnel.</p>';
    } else {
        echo '<p>⚠️ <strong>LocarcPDF introuvable</strong> : le fichier <code>' . esc_html($fpdf_path) . '</code> est manquant.</p>';
    }
...
function locarc_is_equipment_assigned($identifier, $kind, $exclude_contract_id = 0) {
    global $wpdb;
    $t = locarc_tables();
    // $col is set from a hardcoded ternary (never user input); table name from locarc_tables().
...
function locarc_render_contracts() {
    ...
    foreach ($rows as $r) {
        ...
        echo '<tr class="' . esc_attr($row_class) . '" data-id="' . esc_attr($r['id']) . '"'
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- read-only highlight of newly created row, no state change.
...
function locarc_render_imports() {
    ...
    // phpcs:disable WordPress.Security.NonceVerification.Recommended -- read-only success/error notices displayed after redirect from a nonce-verified admin_post handler.
    ...
    if (!empty($_GET['import_error'])) {
        echo '<div class="notice notice-error is-dismissible"><p>❌ Import impossible : ' . esc_html(wp_unslash($_GET['import_error'])) . '</p></div>'; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- value is passed through esc_html() above.
    }
...
add_action('admin_post_locarc_import_members', function() {
    ...
    $count = locarc_import_members_from_csv($_FILES['csv']['tmp_name']); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.InputNotValidated -- tmp_name is a server-generated temp path, never user input.
...
add_action('admin_post_locarc_import_branches', function() {
    ...
    $count = locarc_import_matos_from_csv($_FILES['csv']['tmp_name'], 'branches'); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.InputNotValidated -- tmp_name is a server-generated temp path, never user input.
    if (is_wp_error($count)) {
        wp_safe_redirect(admin_url('admin.php?page=locarc&tab=imports&import_error=' . rawurlencode($count->get_error_message())));
        exit;
    }
...
add_action('admin_post_locarc_import_handles', function() {
    ...
    $count = locarc_import_matos_from_csv($_FILES['csv']['tmp_name'], 'handles'); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.InputNotValidated -- tmp_name is a server-generated temp path, never user input.
    if (is_wp_error($count)) {
        wp_safe_redirect(admin_url('admin.php?page=locarc&tab=imports&import_error=' . rawurlencode($count->get_error_message())));
        exit;
    }
...
function locarc_ajax_save_item() {
    ...
    if ($id > 0) {
        ...
        $wpdb->update($table, $data, ['id'=>$id], $formats, ['%d']); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
        $saved_id = $id;
    } else {
        $wpdb->insert($table, $data, $formats); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
...
function locarc_ajax_get_member_by_licence() {
    ...
    global $wpdb;
    $t = locarc_tables();
    $licence = sanitize_text_field(wp_unslash($_GET['licence'] ?? ''));
...
function locarc_ajax_save_contract() {
    ...
    $id = intval($_POST['id'] ?? 0);
    $licence = sanitize_text_field(wp_unslash($_POST['licence'] ?? ''));
    $type = sanitize_key(wp_unslash($_POST['contract_type'] ?? 'complet'));
    $start = sanitize_text_field(wp_unslash($_POST['start_date'] ?? wp_date('Y-m-d')));
    $end = sanitize_text_field(wp_unslash($_POST['end_date'] ?? wp_date('Y-m-d', strtotime('+1 year', strtotime($start)))));
    $handle_id = sanitize_text_field(wp_unslash($_POST['handle_identifier'] ?? ''));
    $branches_id = sanitize_text_field(wp_unslash($_POST['branches_identifier'] ?? ''));
    // Equipment characteristics (optional; used for 'Prêt' or cached display/PDF)
    $handle_brand = sanitize_text_field(wp_unslash($_POST['handle_brand'] ?? ''));
    $handle_model = sanitize_text_field(wp_unslash($_POST['handle_model'] ?? ''));
    $handle_size = (wp_unslash($_POST['handle_size'] ?? '')) === '' ? null : intval(wp_unslash($_POST['handle_size'])); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- intval() is sufficient sanitization for an integer field.
    $handle_handedness = sanitize_text_field(wp_unslash($_POST['handle_handedness'] ?? ''));

    $branches_brand = sanitize_text_field(wp_unslash($_POST['branches_brand'] ?? ''));
    $branches_model = sanitize_text_field(wp_unslash($_POST['branches_model'] ?? ''));
    $branches_size = (wp_unslash($_POST['branches_size'] ?? '')) === '' ? null : intval(wp_unslash($_POST['branches_size'])); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- intval() is sufficient sanitization for an integer field.
    $branches_power = (wp_unslash($_POST['branches_power'] ?? '')) === '' ? null : intval(wp_unslash($_POST['branches_power'])); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- intval() is sufficient sanitization for an integer field.
    ...
    $caution_amount_raw = wp_unslash($_POST['caution_amount'] ?? ''); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- sanitized via sanitize_text_field() before floatval() on the next line.
    $caution_amount = ($caution_amount_raw === '' ? null : floatval(str_replace(',', '.', sanitize_text_field($caution_amount_raw))));
    $payment_due_1 = sanitize_text_field(wp_unslash($_POST['payment_due_1'] ?? ''));
    $payment_due_2 = sanitize_text_field(wp_unslash($_POST['payment_due_2'] ?? ''));
    $payment_due_3 = sanitize_text_field(wp_unslash($_POST['payment_due_3'] ?? ''));
    $payment_due_4 = sanitize_text_field(wp_unslash($_POST['payment_due_4'] ?? ''));
    ...
    $paid = intval($_POST['is_paid'] ?? 0) ? 1 : 0;
    $custom_price_raw = wp_unslash($_POST['custom_price'] ?? ''); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- sanitized via sanitize_text_field() + floatval() on the next line.
...
function locarc_ajax_update_contract_pricing() {
    ...
    $custom_price_raw = wp_unslash($_POST['custom_price'] ?? ''); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- sanitized via sanitize_text_field() before floatval() on the next line.
    $custom_price = ($custom_price_raw === '' ? null : floatval(str_replace(',', '.', sanitize_text_field($custom_price_raw))));
...
function locarc_ajax_renew_contract() {
    ...
    $licence = sanitize_text_field(wp_unslash($_POST['licence'] ?? ($old['licence'] ?? '')));
    $type = sanitize_key(wp_unslash($_POST['contract_type'] ?? ($old['contract_type'] ?? 'complet')));
    $start = sanitize_text_field(wp_unslash($_POST['start_date'] ?? ($old['end_date'] ?? wp_date('Y-m-d'))));
    $end = sanitize_text_field(wp_unslash($_POST['end_date'] ?? wp_date('Y-m-d', strtotime('+1 year', strtotime($start)))));
    $handle_id = sanitize_text_field(wp_unslash($_POST['handle_identifier'] ?? ($old['handle_identifier'] ?? '')));
    $branches_id = sanitize_text_field(wp_unslash($_POST['branches_identifier'] ?? ($old['branches_identifier'] ?? '')));

    $handle_brand = sanitize_text_field(wp_unslash($_POST['handle_brand'] ?? ($old['handle_brand'] ?? '')));
    $handle_model = sanitize_text_field(wp_unslash($_POST['handle_model'] ?? ($old['handle_model'] ?? '')));
    $handle_size = (wp_unslash($_POST['handle_size'] ?? '')) === '' ? (($old['handle_size'] ?? '') === '' ? null : intval($old['handle_size'])) : intval(wp_unslash($_POST['handle_size'])); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- intval() is sufficient sanitization for an integer field.
    $handle_handedness = sanitize_text_field(wp_unslash($_POST['handle_handedness'] ?? ($old['handle_handedness'] ?? '')));

    $branches_brand = sanitize_text_field(wp_unslash($_POST['branches_brand'] ?? ($old['branches_brand'] ?? '')));
    $branches_model = sanitize_text_field(wp_unslash($_POST['branches_model'] ?? ($old['branches_model'] ?? '')));
    $branches_size = (wp_unslash($_POST['branches_size'] ?? '')) === '' ? (($old['branches_size'] ?? '') === '' ? null : intval($old['branches_size'])) : intval(wp_unslash($_POST['branches_size'])); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- intval() is sufficient sanitization for an integer field.
    $branches_power = (wp_unslash($_POST['branches_power'] ?? '')) === '' ? (($old['branches_power'] ?? '') === '' ? null : intval($old['branches_power'])) : intval(wp_unslash($_POST['branches_power'])); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- intval() is sufficient sanitization for an integer field.

    $payment_method = sanitize_key(wp_unslash($_POST['payment_method'] ?? ($old['payment_method'] ?? '')));
    if (!in_array($payment_method, ['', 'cheque', 'carte_bancaire', 'helloasso', 'especes'], true)) $payment_method = '';
    $caution_amount_raw = wp_unslash($_POST['caution_amount'] ?? ($old['caution_amount'] ?? '')); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- sanitized via sanitize_text_field() before floatval() on the next line.
    ...
    $payment_due_1 = sanitize_text_field(wp_unslash($_POST['payment_due_1'] ?? ($old['payment_due_1'] ?? '')));
    $payment_due_2 = sanitize_text_field(wp_unslash($_POST['payment_due_2'] ?? ($old['payment_due_2'] ?? '')));
    $payment_due_3 = sanitize_text_field(wp_unslash($_POST['payment_due_3'] ?? ($old['payment_due_3'] ?? '')));
    $payment_due_4 = sanitize_text_field(wp_unslash($_POST['payment_due_4'] ?? ($old['payment_due_4'] ?? '')));
    if ($payment_method !== 'cheque') {
        $payment_due_1 = $payment_due_2 = $payment_due_3 = $payment_due_4 = '';
    }
    ...
    $paid = intval($_POST['is_paid'] ?? 0) ? 1 : 0;
    $custom_price_raw = wp_unslash($_POST['custom_price'] ?? ($old['custom_price'] ?? '')); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- sanitized via sanitize_text_field() before floatval() on the next line.
...
function locarc_render_licencies(){
    ...
    $rows = $wpdb->get_results("SELECT * FROM {$table} ORDER BY last_name ASC, first_name ASC", ARRAY_A); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Table name from locarc_tables(), server-defined, never user input.
