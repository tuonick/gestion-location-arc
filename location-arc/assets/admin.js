...
function confirmDelete(label){
  ...
  function openEditItem(kind, id){
    ...
    const title = (id ? 'Modifier' : 'Ajouter') + (isBranches ? ' des branches' : (isHandles ? ' une poignée' : ' un licencié'));
    ...
    const show = (row)=>{
      ...
      if(!id && (kind==='branches' || kind==='handles')){
  ...
  function upsertItemRow(kind, id, row){
    const tableId = (kind==='branches') ? 'locarc-branches-table' : (kind==='handles' ? 'locarc-handles-table' : 'locarc-members-table');
    const $table = $('#'+tableId);
  ...
  function buildItemForm(kind, row){
    ...
    return `
      <form class="locarc-form">
        ...
        <div>
          <label>Taille</label>
          <input type="number" name="size" value="${esc(row.size||'')}" ${isBranches?'min="66" max="70"':'min="23" max="25"'} required />
        </div>
        ${isBranches ? `
          <div>
            <label>Puissance</label>
            <input type="number" name="power" min="16" max="42" step="2" value="${esc(row.power||'')}" required />
          </div>
        ` : extra }
        <div>
          <label>Marque</label>
...
// Renew (open modal prefilled with previous contract)
  ...
  $(document).on('click', '.locarc-tab-group-pill', function(){
    ...
    if (group === 'gestion') {
      $('.locarc-tabs--group-gestion').removeClass('locarc-tabs--hidden');
      $('.locarc-tabs--group-admin').addClass('locarc-tabs--hidden');
    } else {
      $('.locarc-tabs--group-admin').removeClass('locarc-tabs--hidden');
      $('.locarc-tabs--group-gestion').addClass('locarc-tabs--hidden');
    }
  });
