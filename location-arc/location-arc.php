...
Description: Gestion de location d'arcs (contrats, matériel, branches, poignées) avec import Excel.
Version: 0.2.60
...
function locarc_shortcode_my_equipment() {
    ...
    if (!empty($contract['handle_identifier'])) {
        $handle = $wpdb->get_row($wpdb->prepare("SELECT identifier, brand, model, size, handedness FROM {$t['handles']} WHERE identifier=%s", $contract['handle_identifier']), ARRAY_A);
    }
    $branches = null;
    if (!empty($contract['branches_identifier'])) {
        $branches = $wpdb->get_row($wpdb->prepare("SELECT identifier, brand, model, size, power FROM {$t['branches']} WHERE identifier=%s", $contract['branches_identifier']), ARRAY_A);
    }
