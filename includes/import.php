...
// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter, WordPress.WP.AlternativeFunctions.file_system_operations_fclose
...
function locarc_import_rented_from_csv($csv_path) {
    ...
    $first = fgets($fh);
    if ($first === false) { fclose($fh); return new WP_Error('empty', 'Fichier vide'); }
    $delim = (substr_count($first, ';') > substr_count($first, ',')) ? ';' : ',';
    $headers = str_getcsv(trim($first), $delim);
    $map = [];
