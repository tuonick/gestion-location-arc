...
/**
 * PDF generation using Dompdf from the plugin vendor directory.
 */
...
function locarc_default_contract_types() {
    return [
        ...
        'arc_initiation' => ['label' => "Arc d'Initiation", 'price' => 50, 'module' => 'init_bows'],
...
function locarc_contract_types_active() {
    ...
    foreach ($config as $key => $row) {
        ...
        if ($module && !get_option('locarc_enable_' . $module, 0)) continue;
        $result[$key] = $row;
    }
    return $result;
}
...
function locarc_load_dompdf() {
    // Already loaded.
    if (class_exists('Dompdf\\Dompdf')) return true;

    // Search order (Option C: Composer partagé hors plugin en priorité).
    $candidates = [];

    // 1) Path configured by admin in Settings tab.
    $admin_path = trim((string) get_option('locarc_dompdf_autoload_path', ''));
    if ($admin_path !== '') $candidates[] = $admin_path;

    // 2) WordPress root vendor (shared Composer).
    $candidates[] = trailingslashit(ABSPATH) . 'vendor/autoload.php';

    // 3) wp-content/vendor — Composer autoload OR dompdf-specific autoload.inc.php
    //    (covers the case where the dompdf vendor folder was moved to wp-content/vendor).
    ...
    // 4) Plugin-level vendor (Composer or legacy dompdf bundle inside the plugin).
    ...
    return new WP_Error(
        'dompdf_missing',
        'Dompdf introuvable. Lancez <code>composer require dompdf/dompdf</code> à la racine WordPress, ' .
        'ou renseignez le chemin vers <code>autoload.php</code> dans Réglages → Location d\'Arc.'
    );
}

function locarc_render_contract_template($data) {
    $template = trailingslashit(LOCARC_PLUGIN_DIR) . 'templates/contrat-pdf.php';
    if (!file_exists($template)) {
        return new WP_Error('template_missing', 'Template PDF introuvable: ' . $template);
    }
    $html = include $template;
    if (!is_string($html) || trim($html) === '') {
        return new WP_Error('template_invalid', 'Le template PDF doit retourner une chaîne HTML.');
    }
    return $html;
}
...
function locarc_generate_contract_pdf($contract_id) {
    ...
    $data = [
        ...
        'materiel' => (function() use ($c) {
            ...
            if (empty($materiel)) {
                $materiel[] = ['label'=>'—','identifiant'=>'—','marque'=>'','modele'=>'','taille'=>'','lateralite'=>'','puissance'=>''];
            }
            return $materiel;
        })(),
    ...
    $loaded = locarc_load_dompdf();
    if (is_wp_error($loaded)) return $loaded;

    $html = locarc_render_contract_template($data);
    ...
    WP_Filesystem();
    if (!$wp_filesystem || !$wp_filesystem->put_contents($path, $dompdf->output(), FS_CHMOD_FILE)) {
