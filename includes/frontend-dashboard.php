...
function locarc_fe_register_assets() {
    ...
    wp_register_script('locarc-fe-dashboard', plugins_url('../assets/frontend-dashboard.js', __FILE__), [], '2.1.' . (file_exists($js) ? filemtime($js) : LOCARC_VERSION), true);
}
...
function locarc_fe_section_contracts() {
    ...
    <div class="locarc-fe__table-wrap">
      <table class="locarc-fe__table" id="fe-contracts-table">
        ...
        <tbody>
          <?php $row_num = 0; foreach ($rows as $r) : $row_num++; ?>
            <?php
            if (trim((string)($r['first_name'] ?? '')) === '' && trim((string)($r['last_name'] ?? '')) === '') {
...
function locarc_fe_icon($section) {
    ...
    echo $icons[$section] ?? '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="2"/></svg>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
